...
public class HashTable<K, V> {
    ...
    public V get(K key) {
        ...
        if (basket != null) return basket.get(key);
        return null;
    }
    public boolean put(K key, V value) {
        if (baskets.length * LOAD_FACTOR < size) recalculate();
        int index = calculateBasketIndex(key);
        Basket basket = baskets[index];
        if (basket == null) {
            basket = new Basket();
    ...
    private HashTable(int initSize) {
    ...
    // Бакет со связанным списком внутри
    private class Basket {
        ...
        public boolean remove(K key) {
            if (head != null) {
                if (head.value.key.equals(key)) head = head.next;
                else {
                    Node node = head;
        ...
        public boolean add(Entity entity) {
            ...
            if (head != null) {
                Node current = head;
                while (true) {
                    if (current.value.key.equals(entity.key)) return false;
